...
export class Home extends Component {
  ...
  selectProduct = (productFormItem) => {
    this.setState({
      selectedProduct: productFormItem,
    });
  };
  ...
  addToCart = (prod) => {
    ...
    const foundItem = cloneCart.find((item) => {
      return item.products.id === prod.id;
    });
    if (foundItem) {
  ...
  increaseCartItem = (id) => {
    const cloneCart = [...this.state.cart];
    const find = cloneCart.findIndex((item) => {
      return item.products.id === id;
    });
    if (find > -1) {
      cloneCart[find].quantity += 1;
    }
    this.setState({
      cart: cloneCart,
    });
    ...
    return (
      <div className="container">
        ...
        <div className="d-flex flex-row-reverse">
          <button
            className="text-white fs-5 mb-4 px-5 py-2 border btn btn-success"
